<?php
// Vehicle inventory image uploads for PHP Motors
...
// Get thumbnail image path information from images table
function getThumbnailImageInfo($vehicleId)
{
    $db = phpmotorsConnect();
    $sql = "SELECT imgPath, imgName, images.invId, inventory.invId, inventory.invMake, inventory.invModel FROM images JOIN inventory ON images.invId = inventory.invId WHERE images.invId = :id AND imgPath LIKE '%-tn.___%'";
    $stmt = $db->prepare($sql);

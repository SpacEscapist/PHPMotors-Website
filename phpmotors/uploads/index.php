...
switch ($action) {
    case "upload":
        ...
        if ($imageCheck) {
            $message = "<p class='messageFail'>An image by that name already exists.</p>";
        } elseif (empty($invId) || empty($imgName)) {
            $message = "<p class='messageFail'>You must select a vehicle and image file for the vehicle.</p>";
        } else {
            ...
            if ($result) {
                $message = "<p class='messagePass'>The upload succeeded.</p>";
            } else {
                $message = "<p class='messageFail'>Sorry, the upload failed.</p>";
            }
        }
    ...
    case "delete":
        ...
        if ($remove) {
            $message = "<p class='messagePass'>$filename was successfully deleted.</p>";
        } else {
            $message = "<p class='messageFail'>$filename was NOT deleted.</p>";
        }
    ...
    default:
        ...
        if (count($imageArray)) {
            ...
            $imageDisplay = '<p class="messageFail">Sorry, no images could be found.</p>';
        }
